...
class ConversationFlowTests(unittest.TestCase):
    ...
    def test_catalog_by_city(self) -> None:
        ...
        self.assertIn("1. *Playas de Tijuana*", results[-1].reply_text)
        self.assertIn("2. *Zona Río*", results[-1].reply_text)
        self.assertIn("Casa de 3 recámaras, 2 baños, 180m²", results[-1].reply_text)
        self.assertIn("Departamento de 2 recámaras, 1 baño, 95m²", results[-1].reply_text)
        self.assertIn("¿Alguna de estas te interesa para que te comparta más detalles?", results[-1].reply_text)
    ...
    def test_selection_by_zone_sets_property_active(self) -> None:
        ...
        self.assertIn("Excelente elección.", results[-1].reply_text)
        self.assertIn("El departamento en Zona Río tiene 2 recámaras, 1 baño y 95m².", results[-1].reply_text)
        self.assertIn("Su precio de oportunidad es $550K", results[-1].reply_text)
        self.assertNotIn("TJ-ZR-01", results[-1].reply_text)
    ...
    def test_concrete_property_interest_shows_property_without_catalog_dump(self) -> None:
        ...
        self.assertEqual(state.selected_property_id, "TJ-ZR-01")
        self.assertIn("Excelente elección.", results[-1].reply_text)
        self.assertIn("¿Te gustaría saber más sobre esta propiedad?", results[-1].reply_text)
        self.assertNotIn("TJ-ZR-01", results[-1].reply_text)
    ...
    def test_full_flow_uses_natural_commercial_copy_until_handoff(self) -> None:
        ...
        self.assertIn("Genial. Solo para que tengas el panorama completo", results[4].reply_text)
        self.assertIn("Aquí no se trata de una compra tradicional", results[5].reply_text)
